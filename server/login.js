...
function normalizeAndValidateUserId(rawUserId) {
  ...
  if (!/^[a-z0-9]+$/.test(normalizedId)) {
    throw new Error(
      'User ID must only contain letters (a-z) and numbers (0-9).',
    );
  }
  return normalizedId;
}
  ...
  if (storedHash) {
    ...
    if (
      ...
      try {
        ...
        throw new Error(
          'Login failed due to a server error during password check.',
        );
      }
    }
    ...
    if (
      ...
      throw new Error(
        'Login failed: Cannot login with hash, no password set for this user.',
      );
    }
    ...
    try {
      ...
      const message =
        registrationError instanceof Error
          ...
          : 'Server error during initial password registration.';
...
export async function setInfo(rawUserId, info) {
  ...
  if (typeof info !== 'object' || info === null || Array.isArray(info)) {
    return {
      success: false,
      message: 'Info must be a valid JSON object (not an array or null).',
    };
  }
  ...
  if (userData === null) {
    return {
      success: false,
      message: 'User not found or file invalid. Cannot set info.',
    };
  }
  if (typeof userData !== 'object') {
    userData = {};
  ...
  if (saved) {
    ...
    return {
      success: false,
      message: 'Failed to save updated user information.',
    };
  }
}
