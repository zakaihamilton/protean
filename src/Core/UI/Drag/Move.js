...
export function useMoveDrag(enabled = true, options = {}) {
  ...
  const moverCb = useCallback(
    (e, state) => {
      ...
      const left = options?.horizontalLock
        ? Math.floor(state.base.x - state.offset.x)
        : Math.floor(e.clientX - state.offset.x);
      const top = options?.verticalLock
        ? Math.floor(state.base.y - state.offset.y)
        : Math.floor(e.clientY - state.offset.y);
