...
export function useResizeDrag(enabled, direction = 'ltr') {
  ...
  const moverCb = useCallback(
    (e, state) => {
      let width, left;
      const height = Math.floor(
        Math.max(e.clientY - state.offset.y, state.min?.height || 0),
      );
      if (direction === 'ltr') {
        left = state.rect.left;
        width = Math.floor(
          Math.max(e.clientX - state.offset.x, state.min?.width || 0),
        );
      } else {
        width = Math.floor(
          Math.max(
            state.base.x - e.clientX + state.base.width,
            state.min?.width || 0,
          ),
        );
        left = Math.floor(
          state.base.x - width + state.base.width + state.shiftLeft,
        );
      }
      if (state.base.width !== width || state.base.height !== height) {
        state.rect = Object.assign(state.rect, { left, width, height });
        state.dragged = {
          x: e.clientX - state.base.x,
          y: e.clientY - state.base.y,
        };
      } else {
        state.dragged = null;
      }
