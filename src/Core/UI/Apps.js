...
export default function Apps() {
  ...
  useEffect(() => {
    ...
    if (exists) {
      ...
      if (!screenId) {
        screenId = screenManager?.list
          ?.toReversed()
          ?.find((item) => item.appId === id)?.id;
      }
      if (!screenId) {
        screenId = screenManager?.closed
          ?.toReversed()
          ?.find((item) => item.appId === id)?.id;
      }
      if (!screenId) {
      ...
      {
        const screen = screenManager?.closed?.find(
      ...
      {
        const screen = screenManager?.list?.find(
          (item) => item.id === screenId,
        );
        if (screen) {
