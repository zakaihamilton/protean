...
export async function open() {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onblocked = () => {
      reject(
        new Error(
          'Database open blocked, another connection is open with a higher version.',
        ),
      );
    };
  });
...
export async function get(key) {
  ...
  if (!db) {
    throw new Error(
      'StorageLocal not initialized. Call open() before using the storage.',
    );
  }
...
export async function reset() {
  ...
  await new Promise((resolve, reject) => {
    ...
    request.onblocked = () =>
      reject(
        new Error('Database deletion blocked, another connection is open.'),
      );
  });
  if (wasOpen) {
    await open();
