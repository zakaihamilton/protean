function getComponentPermutations(components) {
  ...
  for (let i = 0; i < keys.length; i += 2) {
    const sourceKey = keys[i],
      targetKey = keys[i === keys.length - 1 ? 0 : i + 1];
    const source = components[sourceKey],
      target = components[targetKey];
...
export function testPermutations(components, params = {}) {
  ...
  const permutations = getComponentPermutations(components).map(
    ([source, target, sourceKey, targetKey]) => {
      const sourceParams = params[sourceKey],
        targetParams = params[targetKey];
      const permutation = [
        sourceKey,
        targetKey,
        [source, target],
...
export function testCompare(values) {
  ...
  function deepCompare(value) {
    ...
    if (
      ...
      Object.keys(value).length !== Object.keys(referenceValue).length
    ) {
      return false;
    }
