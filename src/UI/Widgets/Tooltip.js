...
function Tooltip({ title, description, enabled = true }) {
  ...
  const { element } = Container.State.useState() || {};
  ...
  const descriptionLines = useMemo(() => {
    return description?.split('\\n').map((line, index) => {
      return (
        // biome-ignore lint/suspicious/noArrayIndexKey: Static lines from split text
  ...
  return (
    <div ref={tooltip} style={{ ...position }} className={className}>
      <div className={titleClassName}>{title}</div>
      <div className={descriptionClassName}>{descriptionLines}</div>
    </div>
  );
}
