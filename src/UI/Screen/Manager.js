...
function ScreenManager({ children }) {
  ...
  useEffect(() => {
    manager((state) => {
      ...
      state.visible = state.list?.filter(
        (item) => !item.close && !item.minimize,
      );
    });
  }, [manager, manager.list]);

  useEffect(() => {
    const available = manager.focus?.filter(
      (item) => !item.minimize && !item.close,
    );
    ...
    const timer = setTimeout(() => {
      manager((state) => {
        state.focus = [
          ...(state.focus?.filter((item) => item !== focused) || []),
          focused,
        ].filter(Boolean);
...
export function useScreenItem(screen, target) {
  ...
  useEffect(() => {
    if (
      !screen.minimize === !screen.minimized &&
      !screen.close === !screen.closed
    ) {
      return;
    }
